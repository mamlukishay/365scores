/* 365Scores Predictions Cup — interactive dashboard client.
   Reads window.METRICS (injected by the HTML). Self-contained, offline. */
(function () {
  const M = window.METRICS;
  const $ = (s, r = document) => r.querySelector(s);
  const el = (t, c, h) => { const e = document.createElement(t); if (c) e.className = c; if (h != null) e.innerHTML = h; return e; };
  const pct = (x) => Math.round(x * 100) + "%";
  const STATIC = new URLSearchParams(location.search).get("static") === "1"; // screenshot mode
  ...
  Chart.defaults.animation = STATIC ? false : { duration: 650, easing: "easeOutQuart" };
  ...
  // ---- state ----
  const byName = Object.fromEntries(M.players.map((p) => [p.name, p]));
  const names = M.standings.map((s) => s.name);
  const SEL_KEY = "selectedPlayer";
  const savedSel = (() => { try { return localStorage.getItem(SEL_KEY); } catch { return null; } })();
  let selected = (names.includes(savedSel) && savedSel)
  ...
  // ============ confetti ============
  function celebrate() {
    if (STATIC || !window.confetti) return;
